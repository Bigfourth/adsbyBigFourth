...
function XadAdxFirstView(_adUnit, _adSize = [300, 600]) {
  ...
  var html = `<div class="xad-firstview" style="display: block; position: fixed; width: 100%; inset: 0; top: 0px; left: 0px; text-align: center; opacity: 1; background-color: rgba(255, 255, 255, 0.7); visibility: hidden; z-index: 2147483647;">
...
function XadAdxCatfishAuto(_adUnit, _adSize = null, _isDisplay = 0, _pageView = [0], _bottom = 0) {
  ...
  if (pageViewCount == 0) {
    localStorage.setItem('expiry', now.getTime());
  } else if (now.getTime() - Number(localStorage.getItem('expiry')) > 180000) {
    pageViewCount = 0;
    localStorage.setItem('expiry', now.getTime());
  }
  localStorage.setItem('pageViewCount', ++pageViewCount);
