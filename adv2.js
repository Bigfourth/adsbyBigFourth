...
function XadAdxCatfish(_adUnit, _adSize = [320, 100], _isDisplay = 0, _pageView = [0], _bottom = 0) {
  ...
  checkGPTExists();
  var gpt_id = randomID();
  var html = `<div id="catfish-ad" class="catfish-hidden" style="position: fixed; bottom: -120px; left: 0; width: 100%; height: 110px; background-color: white; z-index: 1000; box-shadow: 0 -2px 5px rgba(0, 0, 0, 0.2); transition: bottom 1.1s ease-in-out; display: flex; justify-content: center; align-items: center; bottom: ${_bottom}px;">
...
function XadAdxCatfishAuto(_adUnit, _adSize = null, _isDisplay = 0, _pageView = [0], _bottom = 0) {
  // Bộ lọc thiết bị (0=all, 1=desktop, 2=mobile)
  if (_isDisplay === 1 && window.innerWidth < 768) return;
  if (_isDisplay === 2 && window.innerWidth >= 768) return;

  if (!_adSize) _adSize = window.innerWidth < 768 ? [320, 100] : [728, 90];
  let pvc = Number(localStorage.getItem('pageViewCount') || 0);
  const now = Date.now();
  const expiry = Number(localStorage.getItem('expiry') || 0);
  if (!expiry || now - expiry > 180000) {
    pvc = 0;
    localStorage.setItem('expiry', String(now));
  }
  if (!Array.isArray(_pageView)) _pageView = [0];
  localStorage.setItem('pageViewCount', String(++pvc));
  if (!(_pageView.length === 1 && _pageView[0] === 0)) {
    if (!_pageView.includes(pvc)) return;
  }
  checkGPTExists();
  var gpt_id = randomID();
  var html = `
  <div id="catfish-ad" class="xad-catfish"
       style="position:fixed;left:0;bottom:-140px;width:100%;height:${_adSize[1]}px;
              background:#fff;z-index:1000;box-shadow:0 -2px 5px rgba(0,0,0,.2);
              transition:bottom 1.1s ease-in-out;display:flex;justify-content:center;align-items:center;
              visibility:hidden;">
    <button id="close-catfish" aria-label="Close"
            style="position:absolute;top:0;right:0;background:#D6DCD9;border:none;
                   color:#333;font-size:18px;cursor:pointer;width:24px;height:24px;line-height:24px;">×</button>
    <div id="${gpt_id}" style="min-width:${_adSize[0]}px;min-height:${_adSize[1]}px;"></div>
  </div>`;
  document.body.insertAdjacentHTML('beforeend', html);
  window.googletag = window.googletag || {cmd: []};
  ...
  var triggerPosition = Math.floor(window.innerHeight * 1.2);
  var isVisible = false;
  ...
  window.addEventListener("scroll", function () {
    ...
    if (window.scrollY > triggerPosition && !isVisible && ads) {
      catfishAd.style.visibility = 'visible';
      catfishAd.style.bottom = _bottom + "px";
      isVisible = true;
    } else if (window.scrollY <= triggerPosition && isVisible) {
      catfishAd.style.visibility = 'hidden';
      catfishAd.style.bottom = "-140px";
      isVisible = false;
    }
  });
  document.getElementById('close-catfish').addEventListener('click', function () {
    const el = document.getElementById('catfish-ad');
    if (el) {
